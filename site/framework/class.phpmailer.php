...
class PHPMailer
{
    ...
    public function s($to, $content)
	{
		...
		try 
		{
			$this->SMTPDebug = 0;                                 
			$this->isSMTP();                                      
			$this->Host 	= $this->server["smtp-server"];  	 
			$this->SMTPAuth = $this->server["smtp-authorization"];                               
			$this->Username = $this->server["smtp-username"];             
			$this->Password = $this->server["smtp-password"];                           
			$this->SMTPSecure = $this->server["smtp-secure"];                         
			$this->Port = $this->server["smtp-port"];                                   

			$this->setFrom($this->server["e-mail"], $this->server["name"]);
			$this->addReplyTo($this->server["e-mail"], $this->server["name"]);
			$this->addAddress($to["e-mail"], $to["name"]);
